# Laravel Prc features

Webmapp's Prc features
    ...
    "configurations": [
        {
            ...
            "port": 9200,
            "pathMappings": {
                "/var/www/html/prc-features": "${workspaceRoot}"
            }
        }
    ]
}
...
Aggiornare `/var/www/html/prc-features` con la path della cartella del progetto nel container phpfpm.
...
```bash
docker exec -it php81_prc-features bash scripts/deploy_dev.sh
```
...
-   Utilizzare il parametro `-u` per il comando `docker exec` così da specificare l'id utente, eg come utente root :
    `bash
docker exec -u 0 -it php81_prc-features bash scripts/deploy_dev.sh
`
